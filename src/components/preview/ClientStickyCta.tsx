...
export function ClientStickyCta({ client }: { client: ClientSite }) {
  return (
    <div
      className="fixed inset-x-0 bottom-0 z-50 border-t border-[var(--line)] bg-[color-mix(in_srgb,var(--bg)_95%,transparent)] px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-md md:hidden"
    >
      <div className="mx-auto flex max-w-site gap-2">
        <a
          href={clientTelUrl(client)}
          className="flex flex-1 items-center justify-center rounded-xl border border-[color-mix(in_srgb,var(--ink)_15%,transparent)] py-3 text-sm font-semibold text-[var(--ink)]"
        >
          Anrufen
        </a>
        <a
